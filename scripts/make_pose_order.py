...
def main():
    ...
    a("")
    a("## 受領済みシートからの是正点（2026-09-02 受領の10ポーズシート）")
    a("")
    a("方向性は採用する。ただし次はすべて直す必要がある。実測の根拠は "
      "`reports/art-style-feasibility.md`。")
    a("")
    a("| 項目 | 受領物 | 直すこと |")
    a("|---|---|---|")
    a("| **質感** | 紙の粒子とフェルトの繊維あり（ユニーク色 14,911） | "
      "**フラット描画。ベタ塗りのみ。粒子・繊維・布目・にじみを一切描かない** |")
    a("| 容量 | 20フレームAPNGで 1,179,874 bytes（上限の3.9倍） | "
      "10色フラットなら 154,017 bytes で成立する |")
    a("| 背景 | クリームの紙テクスチャ、透過なし | **完全な透過（RGBA）** |")
    a("| 文字 | タイトル・ラベル・説明文が焼き込み | **画像内に文字を一切入れない** |")
    a("| 構成 | 1枚に10体 | 1体ずつ、または2×2シート |")
    a(f"| 寸法 | 1536×1024 | **{RENDER_PX}×{RENDER_PX}px／1ポーズ** |")
    a("| 感情 | びっくり・どや・いたずら等 | 下記「個別指示」の40種に従う |")
    a("")
    a("**16色へ量子化しただけでは 398,332 bytes で不合格だった。**"
      "容量を支配しているのは色数ではなく画素単位のざらつきである。"
      "また、後処理でフィルタをかけて平坦化するとステッチの輪郭とボタンの目が消えてしまい使えない。"
      "**最初からフラットに描く以外に成立させる方法がない。**")
    a("")
    a("## 共通制作ルール（全枚数に適用）")
    a("")
    a("- **フラット描画。面はすべてベタ塗り。** 紙の粒子・フェルトの繊維・布目・にじみ・"
      "グラデーション・光沢・ぼかし影・半透明グローを一切描かない。"
      "フェルトらしさは『切り口のわずかな不揃い』と『ステッチの形』だけで表す。")
    a(f"- **配色は下記10色のみ。** この外の色を1色も使わない。")
    ...
    a(f"- 本体の全高は **{lock['size_lock']['target_character_height_px']}px"
      f"（720px換算で {lock['size_lock']['target_character_height_px']*4}px）**、"
      f"全幅 **{lock['size_lock']['target_character_width_px']}px"
      f"（同 {lock['size_lock']['target_character_width_px']*4}px）** を基準にする。"
      "大きく描きすぎると大きな動作が安全域に収まらない。")
    a("")
    a("### 2×2シートで生成する場合")
    a("")
    ...
    for h in lock["visual_hooks"]:
        a(f"  - **{h['name']}** — {h['note']}")
    a(f"- 陰影: {lock['shading_rule']}")
    a(f"- 質感: {lock['texture_rule']}")
    a("")
    a("**この1枚が承認されるまで、他の原画を量産しないこと。** 以後この画像が唯一の同一性基準になる。")
    a("")
    a("## 感情はステッチの状態で表す（この商品の核）")
    a("")
    a("表情だけに頼らず、縫い目そのものの状態で気持ちを示す。各ポーズでどの状態にするかは"
      "「個別指示」のエフェクト欄に書いてある。")
    a("")
    a("| 状態 | 見え方 |")
    a("|---|---|")
    for k, v in lock["emotion_stitch_mechanic"]["states"].items():
        a(f"| `{k}` | {v} |")
    a("")
    a("縫い目は小さく離散的な印の集まりなので、そこだけを動かせば毎フレームの差分矩形が小さくなり、"
      "APNG が軽くなる。**感情表現と容量設計が同じ仕組みで両立している。**")
    a("")
    a("## 採寸用「本体のみ版」について")
    a("")
    a("`poses/NNN/P1-body.png` として、P1 から**小物・エフェクトパーツ・飛び出した糸端**を取り除いた版を1枚。")
    a("§5 のサイズ統一は主役だけのマスクで測るため、これが無いと "
      "`reports/scale-audit.csv` が算出できない（小物込みの外接矩形で測ってはならない）。")
    a("首元のリボンと縫い合わせ線は本体の一部として**残す**。除くのは小物・エフェクトパーツと、飛び出した糸端だけ。")
    a("")
    a("## 記号 33〜40 について")
    a("")
    ...
    a("- 各パーツの縁を**ブランケットステッチでかがり、糸端を1本だけ飛び出させる**（シリーズの署名）")
    ...
    a("# 貼ってすぐ使える生成プロンプト")
    a("")
    a("別環境の画像生成ツールへそのまま貼れる形にしてある。"
      "`style-lock.json` から生成しているため、設定を変えればここも自動で追従する。")
    a("")
    a("## A. 基準キャラクター `character-master.png`")
    a("")
    a("```text")
    a("フェルトを縫い合わせて作られたカワウソのキャラクター。正面向き、直立、無表情に近い基本状態。")
    a("")
    a("【最重要・描画スタイル】")
    a("完全にフラットなベクター風のベタ塗り。面はすべて単色で塗る。")
    a("紙の粒子、布の繊維、フェルトの毛羽立ち、布目、にじみ、ざらつき、ノイズ、")
    a("グラデーション、光沢、ぼかし影、半透明の光を一切描かない。")
    a("水彩やテクスチャブラシの風合いを出さない。写真的な質感を出さない。")
    a("陰影は各色につき1段のベタ面のみ。光源は左上45度。")
    a("")
    a("【配色 — 次の10色だけを使う。他の色を1色も使わない】")
    for k, v in {**lock["base_colors"], **lock["effect_colors"]}.items():
        a(f"  {v}  {k}")
    a("")
    a("【造形】")
    a(f"  {lock['silhouette']}")
    a(f"  頭身: {lock['proportions']['note']}")
    a(f"  顔: {lock['face_rules']}")
    a(f"  目: {lock['eye_rules']}")
    a(f"  口: {lock['mouth_rules']}")
    a("")
    a("【必ず入れる4つの特徴】")
    for i, h in enumerate(lock["visual_hooks"], 1):
        a(f"  {i}. {h['name']} — {h['note']}")
    a("")
    a("【輪郭】")
    a(f"  {lock['line_width_rule']}")
    a("")
    a("【出力条件】")
    a(f"  {RENDER_PX}×{RENDER_PX}px、背景は完全な透過（アルファチャンネルあり）。")
    a("  画像内に文字・ラベル・タイトル・説明・透かしを一切入れない。")
    a("  1枚に1体だけ。複数のポーズを並べない。")
    a(f"  キャラクター本体の全高を画像の高さの約"
      f"{lock['size_lock']['target_character_height_px']*100//180}%に収め、")
    a("  上下左右に画像の高さの約4.5%以上の余白を残す。")
    a("```")
    a("")
    a("## B. 各ポーズ（承認後に使う。`<>` を個別指示の内容へ置き換える）")
    a("")
    a("```text")
    a("承認済みの基準キャラクター（添付）と同一のキャラクターを、別のポーズで描く。")
    a("顔・体型・耳・縫い合わせ線・ボタンの目・リボン・配色・縫い目の太さと間隔を一切変えない。")
    a("")
    a("ポーズ: <個別指示の「描く内容」をそのまま貼る>")
    a("表情: <個別指示の「第1フレームの表情」>")
    a("縫い目の状態: <tight / loose / frayed / red / wave のいずれか>")
    a("")
    a("【描画スタイル】基準キャラクターと完全に同一。フラットなベタ塗り、上記10色のみ、")
    a("粒子・繊維・グラデーション・光沢・ぼかしを一切描かない。陰影は1段。光源は左上45度。")
    a("")
    a("【出力条件】")
    a(f"  {RENDER_PX}×{RENDER_PX}px、背景は完全な透過。文字を入れない。1枚に1ポーズ。")
    a("  キャラクター本体の見かけの大きさ・中心位置・足元の高さを基準キャラクターと揃える。")
    a("  ポーズに合わせて拡大縮小しない。")
    a("```")
    a("")
    a("## C. 生成後にこちらで自動検査する項目")
    a("")
    a("届いた原画に対し、Claude 側で次を機械検査する。不合格なら再生成を依頼する。")
    a("")
    a("```bash")
    a("python3 scripts/check_incoming_art.py poses/001/*.png \\")
    a("    --json reports/incoming-art-audit.json")
    a("```")
    a("")
    a("| 検査 | 基準 |")
    a("|---|---|")
    a("| 透過 | アルファチャンネルがあり、背景が完全透過であること |")
    a("| 色数 | 10色パレット外の色が支配的に含まれないこと |")
    a("| ざらつき | 不透明部のユニーク色数。テクスチャが残っていれば数千〜万単位になる |")
    a("| 寸法 | 720×720px |")
    a(f"| 本体サイズ | 全高が基準の ±{lock['size_lock']['tolerances']['height_pct']}% 以内 |")
    a("| 安全域 | 上下左右に32px（180px換算で8px）以上の余白 |")
    a("| 焼き込み文字 | 画像内に文字が無いこと（目視） |")
    a("")
    a("---")
    a("")
    a("# 個別指示")
